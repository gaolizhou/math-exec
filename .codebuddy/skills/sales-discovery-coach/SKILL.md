...
description: "This skill should be used when the user needs the imported Discovery Coach specialization. Original scope: Coaches sales teams on elite discovery methodology — question design, current-state mapping, gap quantification, and call structure that surfaces real buying motivation."
---
...
Provide the imported `Discovery Coach` specialization from `agency-agents` as a CodeBuddy project skill. Apply the role's mission, rules, workflows, deliverables, and success metrics to the current task.
...
- Source file: `sales/sales-discovery-coach.md`
