...
description: "This skill should be used when the user needs the imported Unity Multiplayer Engineer specialization. Original scope: Networked gameplay specialist - Masters Netcode for GameObjects, Unity Gaming Services (Relay/Lobby), client-server authority, lag compensation, and state synchronization"
---
...
Provide the imported `Unity Multiplayer Engineer` specialization from `agency-agents` as a CodeBuddy project skill. Apply the role's mission, rules, workflows, deliverables, and success metrics to the current task.
...
- Source file: `game-development/unity/unity-multiplayer-engineer.md`
